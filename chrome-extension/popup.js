...
async function detectJournalFormat() {
  ...
  try {
    const result = await chrome.runtime.sendMessage({ action: 'getLogseqConfig' });
...
async function testConnection() {
  ...
  await chrome.runtime.sendMessage({ action: 'saveSettings', settings: tempSettings });
...
async function detectGraphName() {
  ...
  const result = await chrome.runtime.sendMessage({ action: 'getGraphName' });
